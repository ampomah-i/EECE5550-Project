...
parent_dir = os.path.dirname(os.path.realpath('/home/rushi/mobile_robotics/final_project/car-racing/src/car_racing/control'))
        ...
        if args["simulation"]:
            ...
            if args["sim_replay"]:
                ...
                for index in range(0, num_veh):
                    veh_name = "car" + str(index + 1)
                    s_func = 0.1 * random.randint(0, 10) * t_symbol + 3 + random.randint(0, 14)
                    vehicles[index].set_state_curvilinear_func(
                        t_symbol,
                        s_func,
                        t_symbol,
                ...
                vehicles = set_up_other_vehicles(track, num_veh)
                # for index in range(0, num_veh - 1):
                    
                #     veh_name = "car" + str(index + 1)
                #     vehicle = offboard.NoDynamicsModel(name=veh_name, param=base.CarParam(edgecolor="orange"))
                #     vehicle.set_track(track)
                #     vehicle.set_state_curvilinear_func(t_symbol, 1 * t_symbol + 3.0, t_symbol)
                #     vehicle.start_logging() 
                        ...
                        if iter == 5:
                            if save_lmpc_traj:
                                with open(
                                    ...
                                    pickle.dump(ego, handle, protocol=pickle.HIGHEST_PROTOCOL)
                # This is the additionn that will save animations from all the laps ////////////////////////////
                # if args["diff_alpha"]:
                #     file_name = "racing_game_alpha_" + str(alpha)
                # elif args["random_other_agents"]:
                #     file_name = "racing_game_random"
                # else:
                #     file_name = "racing_game_" + track_layout + 'lap_number_' + str(iter)
                # simulator.animate(filename=file_name, ani_time=400, racing_game=True)
                # Addition ends here //////////////////////////////////////
...
def set_up_ego(timestep, track):
    ...
    pid_controller = offboard.PIDTracking(vt=1.6, eyt=0.0)
    ...
    mpc_lti_param = base.MPCTrackingParam(vt=1.6, eyt=0.0)
